...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    if (session.payment_status !== 'paid') return new Response('Not paid', { status:400 });
    ...
    const lineItems = await stripe.checkout.sessions.listLineItems(session.id, { limit:100, expand:['data.price'] });
    ...
    for (const item of lineItems.data) {
      ...
      const variantId = Number(price?.metadata?.variant_id || 0);
      if (!variantId) throw new Error(`Missing variant_id metadata for checkout item ${item.id}`);
      const quantity = Number(item.quantity || 1);
      const variant = await env.DB.prepare('SELECT id, stock_quantity FROM variants WHERE id = ?').bind(variantId).first<{id:number;stock_quantity:number}>();
      ...
      await env.DB.prepare('INSERT INTO order_items (order_id, variant_id, quantity, unit_price) VALUES (?,?,?,?)').bind(order.id, variant.id, quantity, Number(price?.unit_amount || 0)).run();
    }
